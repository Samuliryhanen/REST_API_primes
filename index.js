...
app.get('/myapi', function (req, res) {
    ...
    if(action.localeCompare('sumandcheck') === 0){
        ...
        let sum = calculate_sum(numbers);  //parse metodi tapahtuu calculate function sisällä, joten sitä ei erikseen kutsuta tässä.
        let is_prime = check_prime(sum);    
    ...
    if(action.localeCompare('checkprime') === 0){
        let number = "0";
        let number_list = parse_integers(req.query.number); // Jos syöte on muotoa  " 00134 1023210 3 adkaod 213"
        for(let i of number_list){                          // Karsitaan kaikki ylimääräiset merkit ja yhdistetään jono numeroksi  
            number +=i;
        }        
        let is_prime = check_prime(number);    
...
function calculate_sum(numbers){
    ...
    let list = parse_integers(numbers);
    let sum = 0;
    if(list.length > 0){
        for(let i of list){
            sum += parseFloat(i,10);    
